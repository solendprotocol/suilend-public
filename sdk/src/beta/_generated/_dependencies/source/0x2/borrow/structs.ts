...
export function isBorrow(type: string): boolean {
  ...
  return type === `${PKG_V23}::borrow::Borrow`;
}
...
export class Borrow implements StructClass {
  static readonly $typeName = `${PKG_V23}::borrow::Borrow`;
  static readonly $numTypeParams = 0;

  readonly $typeName = Borrow.$typeName;

  readonly $fullTypeName: `${typeof PKG_V23}::borrow::Borrow`;
  ...
  private constructor(typeArgs: [], fields: BorrowFields) {
    this.$fullTypeName = composeSuiType(
      Borrow.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V23}::borrow::Borrow`;
...
export function isReferent(type: string): boolean {
  type = compressSuiType(type);
  return type.startsWith(`${PKG_V23}::borrow::Referent` + "<");
}
...
export class Referent<T extends TypeArgument> implements StructClass {
  static readonly $typeName = `${PKG_V23}::borrow::Referent`;
  static readonly $numTypeParams = 1;

  readonly $typeName = Referent.$typeName;

  readonly $fullTypeName: `${typeof PKG_V23}::borrow::Referent<${ToTypeStr<T>}>`;
  ...
  private constructor(typeArgs: [ToTypeStr<T>], fields: ReferentFields<T>) {
    this.$fullTypeName = composeSuiType(
      Referent.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V23}::borrow::Referent<${ToTypeStr<T>}>`;
    this.$typeArgs = typeArgs;
  ...
  static reified<T extends Reified<TypeArgument, any>>(
    ...
    return {
      ...
      fullTypeName: composeSuiType(
        Referent.$typeName,
        ...[extractType(T)],
      ) as `${typeof PKG_V23}::borrow::Referent<${ToTypeStr<ToTypeArgument<T>>}>`,
