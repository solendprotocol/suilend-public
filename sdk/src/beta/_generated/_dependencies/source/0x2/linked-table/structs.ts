...
export function isLinkedTable(type: string): boolean {
  ...
  return type.startsWith(`${PKG_V23}::linked_table::LinkedTable` + "<");
}
...
{
  static readonly $typeName = `${PKG_V23}::linked_table::LinkedTable`;
  static readonly $numTypeParams = 2;

  readonly $typeName = LinkedTable.$typeName;

  readonly $fullTypeName: `${typeof PKG_V23}::linked_table::LinkedTable<${ToTypeStr<K>}, ${PhantomToTypeStr<V>}>`;
  ...
  private constructor(
    ...
    this.$fullTypeName = composeSuiType(
      ...
      ...typeArgs,
    ) as `${typeof PKG_V23}::linked_table::LinkedTable<${ToTypeStr<K>}, ${PhantomToTypeStr<V>}>`;
  ...
  >(
    ...
    return {
      ...
      ) as `${typeof PKG_V23}::linked_table::LinkedTable<${ToTypeStr<ToTypeArgument<K>>}, ${PhantomToTypeStr<ToPhantomTypeArgument<V>>}>`,
...
export function isNode(type: string): boolean {
  type = compressSuiType(type);
  return type.startsWith(`${PKG_V23}::linked_table::Node` + "<");
}
...
export class Node<K extends TypeArgument, V extends TypeArgument>
  implements StructClass
{
  static readonly $typeName = `${PKG_V23}::linked_table::Node`;
  static readonly $numTypeParams = 2;

  readonly $typeName = Node.$typeName;

  readonly $fullTypeName: `${typeof PKG_V23}::linked_table::Node<${ToTypeStr<K>}, ${ToTypeStr<V>}>`;
  ...
  private constructor(
    ...
    this.$fullTypeName = composeSuiType(
      Node.$typeName,
      ...typeArgs,
    ) as `${typeof PKG_V23}::linked_table::Node<${ToTypeStr<K>}, ${ToTypeStr<V>}>`;
    this.$typeArgs = typeArgs;
  ...
  >(K: K, V: V): NodeReified<ToTypeArgument<K>, ToTypeArgument<V>> {
    return {
      ...
      fullTypeName: composeSuiType(
        Node.$typeName,
        ...[extractType(K), extractType(V)],
      ) as `${typeof PKG_V23}::linked_table::Node<${ToTypeStr<ToTypeArgument<K>>}, ${ToTypeStr<ToTypeArgument<V>>}>`,
      typeArgs: [extractType(K), extractType(V)] as [
        ToTypeStr<ToTypeArgument<K>>,
