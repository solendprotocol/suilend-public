...
function Page() {
  ...
  const swap = async (deposit?: boolean) => {
    ...
    try {
      ...
      tx = Transaction.from(hopTx.transaction as unknown as Transaction);
      tx.setGasBudget("" as any); // Set to dynamic

      if (isDepositing) {
        if (!hopTx.output_coin) throw new Error("Missing coin to deposit");
